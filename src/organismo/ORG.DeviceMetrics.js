...
class ORGDeviceMetrics {
    ...
    /***
     * Finds the device in ORGDevicesData that matches the screen points passed in argument.
     * @param size in screen points
     * @returns {ProductName String}
     */
    static deviceWithScreenPoints(size) {
        for (let key in ORGDevicesData) {
            if (ORGDevicesData[key].Points.X == size.width && ORGDevicesData[key].Points.Y == size.height) {
                return ORGDevicesData[key].ProductName;
            }
        }
        return "unknown";
    }
...
module.exports = { ORGDeviceMetrics, ORGDevicesData }
